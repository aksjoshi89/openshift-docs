...
[IMPORTANT]
====
Updating or replacing an existing custom API server certificate triggers a configuration change that is synchronized by the Machine Config Operator (MCO). Because the API server certificate is part of the cluster's core trust material, this process will trigger a rolling reboot of the control plane (master) nodes in most environments to ensure the host-level services recognize the new certificate. Plan for a maintenance window that accommodates sequential node reboots.
====
...
[NOTE]
====
To ensure the `kube-apiserver` operator and the Machine Config Operator correctly detect a certificate renewal, it is recommended to create a secret with a new name (for example, `api-custom-cert-v2`) rather than overwriting the existing secret.
====
+
+
[source,terminal]
...
. Monitor the Machine Config Pools to verify that the control plane nodes have successfully applied the change and finished rebooting.
+
[source,terminal]
----
$ oc get mcp master
----
+
An update is successful when the `UPDATED` column for the master pool returns to `True` and `UPDATING` is `False`.